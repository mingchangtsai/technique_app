...
# app_rubric_v19.R
# U20 Technique Checklist — Clean build with U16 overrides + duplicate confirm + spinnerless inputs + live validation
# - All four subtechniques (Offset, One-Skate, Diagonal Stride, Double Pole)
# - U16-specific rubric for all four subtechniques (generic override switch)
# - Vertical button selectors; logos at top
# - Clear on Athlete/Date/Subtech/Age changes
# - Manual prefill (Athlete + Sex + Age + Subtech) preferring Date
# - Recent unique 10 list
# - Save message: "Saved to database"
# - Duplicate detection with confirmation modal
# - Hide numeric spinners; live client-side warning for non-integer or out-of-range (0–5)
...
# Helper: safe id for input names
...
# ---- U16 rubric overrides (all four subtechniques, generic switch) ----
rubric_u16 <- list(
  "Offset" = list(
    "Power Position" = c(
...
server <- function(input, output, session) {
  ...
  master_df <- reactiveVal({
    if (file.exists(master_csv)) {
      ...
      tibble(
        ...
        Submitted_At = character(), Subtechnique = character(),
        Area = character(), Item = character(), Score = integer(), Comment = character()
      )
    }
  })

  # Pending save for duplicate confirmation
  ...
  # Helper: choose current rubric for subtech + age
  ...
  # UI for selected subtech (re-renders on age change)
  ...
  # Clear all item inputs for current subtech
  ...
  # Prefill (manual) with quad-match, ordered by Date desc then Submitted_At
  do_prefill <- function() {
  ...
  observeEvent(input$confirm_replace, {
    ...
    df <- master_df()
    df_new <- dplyr::filter(df, !(Athlete == key_vals$Athlete &
                                  Sex == key_vals$Sex &
                                  Age_Group == key_vals$Age_Group &
                                  Date == key_vals$Date &
                                  Subtechnique == key_vals$Subtechnique))
