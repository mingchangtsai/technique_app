...
# app.R — CCBC Technique Checklist
# Reads athlete names from csi.mt_roster and saves submissions to
# csi.technique_checklist (see database/setup_technique.R). The Google
# Sheet / Apps Script bridge is retired.
...
# ---- Load athlete names + past submissions from the database -----------------
# A load failure must NOT abort startup: Connect Cloud only injects the
# CCBC_DB_PWD variable into a successfully published app, so the app has to
# come up without the database and retry per session (a transient DB outage
# then shows a banner instead of killing the process).
load_tc_data <- function() {
  tc_with_con(function(con) {
    list(athletes = tc_load_athletes(con), all = tc_load_all(con))
  })
}
tc_data <- tryCatch(load_tc_data(), error = function(e) e)
...
server <- function(input, output, session) {
  ...
  # If the load failed at process start (or a previous session), retry once
  # per new session -- picks up a newly set password or a recovered database
  if (inherits(tc_data, "error")) {
    tc_data <<- tryCatch(load_tc_data(), error = function(e) e)
  }
  db_ok <- !inherits(tc_data, "error")

  empty_master <- tibble::tibble(
  ...
  master_df <- reactiveVal(
    if (db_ok) tibble::as_tibble(tc_data$all) else empty_master
  )

  # Reload past submissions after a save; keep the current copy on failure
  refresh_master <- function() {
    fresh <- tryCatch(tc_with_con(tc_load_all), error = function(e) NULL)
    if (!is.null(fresh)) master_df(tibble::as_tibble(fresh))
  }

  output$db_banner <- renderUI({
    if (db_ok) return(NULL)
    div(style = paste0("border:1px solid #dc2626; background:#fef2f2; color:#991b1b;",
                       "border-radius:10px; padding:12px 16px; margin-bottom:12px;"),
        strong("Database unavailable — submissions are disabled. "),
        conditionMessage(tc_data),
  ...
  # Blank the score/comment inputs of the current subtechnique after a save
  reset_current_inputs <- function() {
    if (is.null(input$subtech)) return(invisible(NULL))
    st <- input$subtech
    st_list <- current_st_list()
    for (area in names(st_list)) {
      items <- st_list[[area]]
      for (i in seq_along(items)) {
        updateNumericInput(session, paste0("score_", safe_id(st), "_", safe_id(area), "_", i), value = NA_real_)
        updateTextInput(session, paste0("comment_", safe_id(st), "_", safe_id(area), "_", i), value = "")
      }
    }
  }

  # Write to the database (insert or replace); returns TRUE on success and
  # reports the outcome in the status box
  save_to_db <- function(out, replace = FALSE) {
    output$status <- renderText("Saving…")
    ok <- tryCatch({
      tc_with_con(function(con) if (replace) tc_replace(con, out) else tc_insert(con, out))
      TRUE
    }, error = function(e) {
      output$status <- renderText(paste0("Error: could not save to database | ", conditionMessage(e)))
      FALSE
    })
    if (ok) {
      output$status <- renderText("Saved to database")
      refresh_master()
      reset_current_inputs()
      output$prefill_msg <- renderUI(NULL)
    }
    ok
  }
  ...
  observeEvent(input$submit, {
    errs <- c()
    if (!db_ok) errs <- c(errs, "Database unavailable — cannot save.")
